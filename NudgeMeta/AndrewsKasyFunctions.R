#This is the functions needed for Andrews Kasy estimator.
library(ggplot2)
library(reshape2)

#===================================================================
#Robust variance function code
...
#===================================================================
#this is the code to run for getting the main estinates.  It uses Robust variance function inside
...
#T powers function
#Uses T values to make a mtrix of dummies with 1 when T value in cutoff
...
#Main function that estimates it all

metastudies_estimation=function(X,sigma, cutoffs,symmetric, model="normal", cluster_ID){
  ...
  if (model=="normal"){
    ...
    Psihat0 = c(0,1, rep(1,length(cutoffs))) #starting values for LLH
  } else if (model=="t"){
    LLH <- function (Psi) VariationVarianceLogLikelihood(Psi[1], Psi[2], c(Psi[-c(1,2,3)],  1),cutoffs,symmetric, X, sigma, Tpowers, df=Psi[3]) #changed from Psi[3]
  ...
  Var_robust<-RobustVariance(stepsize, nn, Psihat, LLH, cluster_ID = cluster_ID);
...
#====================================================================================
#Finally the code to make plots

estimates_plot1<-function(X, sigma, cutoffs, symmetric, estimates, model="normal"){
  ...
  ggplot(dens[dens$variable == "density of true effect",], aes(x = z, y = value)) + theme_bw() +
    geom_line(size=1.5, color="#004488") +
    # geom_vline(xintercept = estimates$Psihat[1], linetype = "longdash", color = "grey", size =1.5) + 
...
#=======================================================
estimates_plot2<-function(X, sigma, cutoffs, symmetric, estimates, model="normal"){
  ...
  ggplot(dens[dens$variable == 'publication probability',], aes(x = z, y = value)) + theme_bw() +
    geom_line(size=1.5, color="#004488") +
    xlab(label = "Z-score") +
    
    ylab(label = "Publication \nProbablity") +
